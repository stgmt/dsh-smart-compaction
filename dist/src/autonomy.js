...
import { dshHome, findShippedPresetRoots, patchCompositionFile, patchKnownPresetFiles, removeLeftoverSmartPreset, writeHomeDisablePatch, } from "./preset-overlay.js";
export const name = "dsh-smart-compaction-autonomy";
function logStatus(ctx, path, status) {
    if (status === "patched")
        ctx.logger.info("smart-compaction: overlay %s", path);
    if (status === "fail")
        ctx.logger.warn("smart-compaction: overlay failed %s", path);
}
async function patchRoster(ctx, roster, id) {
    const rows = await roster.list();
    const targets = id ? rows.filter((row) => row.id === id) : rows;
    const files = targets.length > 0 ? targets : rows;
    for (const row of files)
        logStatus(ctx, row.path, patchCompositionFile(row.path));
}
function wrapRoster(ctx, roster) {
    ...
    for (const root of roots) {
        ...
        try {
            const watcher = watch(root, { recursive: true }, (_event, filename) => {
                ...
                if (!name.replaceAll("\\", "/").endsWith("agent.cordis.yml"))
                    return;
...
export function apply(ctx, options = {}) {
    if (options.boot !== false) {
        ...
        if (boot.patched > 0)
            ctx.logger.info("smart-compaction: overlaid %s preset file(s)", boot.patched);
        if (boot.failed > 0)
            ctx.logger.warn("smart-compaction: overlay failed on %s file(s)", boot.failed);
    }
    ctx.inject(["agentPresets"], (scoped) => {
        const roster = scoped.get("agentPresets");
        if (roster === undefined)
            return;
