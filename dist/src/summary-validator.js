import { SummaryValidationError } from "./errors.js";
...
export function validateSummary(summary, inputTokenCount, estimateTokens) {
    const text = summary.trim();
    if (!text)
        throw new SummaryValidationError("summary is empty");
    const summaryTokens = estimateTokens(text);
    if (summaryTokens >= inputTokenCount) {
        throw new SummaryValidationError(`summary is not smaller than the input chunk (${summaryTokens} >= ${inputTokenCount})`);
    }
    const present = new Set();
    for (const line of text.split(/\r?\n/)) {
        const key = headingKey(line);
        for (const section of REQUIRED_SUMMARY_SECTIONS) {
            if (key === section.toLowerCase())
                present.add(section);
        }
    }
