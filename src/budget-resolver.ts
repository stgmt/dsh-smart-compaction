...
export function heuristicTokenCount(message: CompactionMessage): number {
  let chars = 0;
  const walk = (blocks: CompactionMessage["content"]): void => {
...
export function resolveBudget(input: {
  ...
  const maxInputTokens = Math.max(
    1024,
    contextWindow - systemTokens - toolTokens - INSTRUCTION_RESERVE_TOKENS - safety,
  );
  return { contextWindow, maxInputTokens, conservative };
}

export function tokensOf(messages: readonly CompactionMessage[], estimate: TokenEstimator): number {
