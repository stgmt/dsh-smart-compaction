...
import type { AtomicBlock, Chunk, CompactionMessage, TokenEstimator } from "./types.ts";
import { tokensOf } from "./budget-resolver.ts";
...
export function splitOverflowChunk(
  chunk: Chunk,
  blocks: readonly AtomicBlock[],
  ...
  if (span.length < 2) {
    throw new OverflowSplitError(
      "context overflow on an atomic tool-paired block that cannot be split further",
    );
  }
  const mid = Math.ceil(span.length / 2);
  const leftBlocks = span.slice(0, mid);
  const rightBlocks = span.slice(mid);
  const leftMessages: CompactionMessage[] = leftBlocks.flatMap((block) => block.messages);
  const rightMessages: CompactionMessage[] = rightBlocks.flatMap((block) => block.messages);
  const left: Chunk = {
    index: chunk.index,
